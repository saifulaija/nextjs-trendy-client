import assets from "@/app/assets";

export const items = [
  {
    label: "MEN'S",
    key: "men",
    children: [
      { label: "BOOT", key: "man-boot", image: assets.images.casual },
      { label: "LOAFER", key: "man-loafer", image: assets.images.casual },
      { label: "SANDALS", key: "man-sandal", image: assets.images.casual },
      { label: "CASUAL", key: "man-casual", image: assets.images.casual },
      { label: "FORMAL", key: "man-formal", image: assets.images.casual },
      { label: "SPORTS", key: "man-sport", image: assets.images.casual },
    ],
  },
  {
    label: "WOMEN'S",
    key: "women",
    children: [
      { label: "BOOT", key: "women-boot", image: assets.images.casual },
      { label: "LOAFER", key: "women-loafer", image: assets.images.casual },
      { label: "CASUAL", key: "women-casual", image: assets.images.casual },
      { label: "SANDALS", key: "women-sandal", image: assets.images.casual },
      { label: "FORMAL", key: "women-formal", image: assets.images.casual },
      { label: "SPORTS", key: "women-sport", image: assets.images.casual },
    ],
  },
  {
    label: "KID'S",
    key: "kid",
    children: [
      { label: "CASUAL", key: "kid-casual", image: assets.images.casual },
      { label: "FORMAL", key: "kid-formal", image: assets.images.casual },
      { label: "SPORT", key: "kid-sport", image: assets.images.casual },
    ],
  },
];

export const man = [
  {
    label: "BOOT",
    href: "/product/man/man-boot",
    image: assets.images.casual,
    description: "Sturdy and stylish boots perfect for any adventure.",
  },
  {
    label: "LOAFER",
    href: "/product/man/man-loafer",
    image: assets.images.casual,
    description: "Elegant loafers for a smart and sophisticated look.",
  },
  {
    label: "SANDALS",
    href: "/product/man/man-sandal",
    image: assets.images.casual,
    description: "Comfortable sandals for easy, breezy days.",
  },
  {
    label: "CASUAL",
    href: "/product/man/man-casual",
    image: assets.images.casual,
    description: "Everyday casual shoes that blend comfort and style.",
  },
  {
    label: "FORMAL",
    href: "/product/man/man-formal",
    image: assets.images.casual,
    description: "Polished formal shoes to complete your sharp look.",
  },
  {
    label: "SPORTS",
    href: "/product/man/man-sport",
    image: assets.images.casual,
    description: "High-performance sports shoes for the active man.",
  },
];



export const women = [
  {
    label: "HEEL",
    href: "/product/women/women-heel",
    image: assets.images.casual,
    description: "Chic heels that elevate your style with every step.",
  },
  {
    label: "FLAT",
    href: "/product/women/women-flat",
    image: assets.images.casual,
    description: "Graceful flats that combine comfort with elegance.",
  },
  {
    label: "SANDALS",
    href: "/product/women/women-sandal",
    image: assets.images.casual,
    description: "Light and trendy sandals for a carefree look.",
  },
  {
    label: "CASUAL",
    href: "/product/women/women-casual",
    image: assets.images.casual,
    description: "Versatile casual shoes for your daily wardrobe.",
  },
  {
    label: "FORMAL",
    href: "/product/women/women-formal",
    image: assets.images.casual,
    description: "Sophisticated formal shoes for any special occasion.",
  },
  {
    label: "SPORTS",
    href: "/product/women/women-sport",
    image: assets.images.casual,
    description: "Dynamic sports shoes for the active woman.",
  },
];


export const kids = [
  {
    label: "SNEAKERS",
    href: "/product/kids/kids-sneaker",
    image: assets.images.casual,
    description: "Comfortable and stylish sneakers for active kids.",
  },
  {
    label: "SANDALS",
    href: "/product/kids/kids-sandal",
    image: assets.images.casual,
    description: "Light and breezy sandals perfect for playtime.",
  },
  {
    label: "BOOTS",
    href: "/product/kids/kids-boot",
    image: assets.images.casual,
    description: "Rugged boots that keep little feet protected.",
  },
  {
    label: "CASUAL",
    href: "/product/kids/kids-casual",
    image: assets.images.casual,
    description: "Everyday casual shoes for fun and comfort.",
  },
  {
    label: "SPORTS",
    href: "/product/kids/kids-sport",
    image: assets.images.casual,
    description: "Sporty shoes designed to keep up with kids' energy.",
  },
  {
    label: "FORMAL",
    href: "/product/kids/kids-formal",
    image: assets.images.casual,
    description: "Elegant formal shoes for young trendsetters.",
  },
];




export const accessories = [
  {
    label: "Man'S ACCESSORIES",
    href: "/product/accessories/man-accessories",
    image: assets.images.casual,
    description:
      "Discover timeless accessories for men that elevate any look, from classic watches to stylish belts.",
  },
  {
    label: "WOMEN'S ACCESSORIES",
    href: "/product/accessories/women-accessories",
    image: assets.images.casual,
    description:
      "Explore elegant accessories for women, including chic jewelry and stylish handbags to complete your outfit.",
  },
];




